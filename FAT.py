...
parser.add_argument(
    "--dataset", type=str, default="cifar10", help="choose from cifar10,svhn"
)
parser.add_argument(
    "--rand_init",
    type=bool,
...
parser.add_argument(
    "--out_dir", type=str, default="./FAT_results", help="dir of output"
)
parser.add_argument(
    "--resume", type=str, default="", help="whether to resume training, default: None"
)
...
class ModelTrain:
    def train(this, model, train_loader, optimizer, tau):
    ...
    def adjust_tau(this, epoch, dynamictau):
        tau = args.tau
        if dynamictau:
            if epoch <= 50:
                tau = 0
    ...
    def adjust_learning_rate(this, optimizer, epoch):
    ...
    def save_checkpoint(this, state, checkpoint=out_dir, filename="checkpoint.pth.tar"):
    ...
    def load_data(this):
        ...
        if args.dataset == "cifar10":
            ...
            this.train_loader = torch.utils.data.DataLoader(
                trainset, batch_size=128, shuffle=True, num_workers=2
            )
            testset = torchvision.datasets.CIFAR10(
                root="./data", train=False, download=True, transform=transform_test
            )
            this.test_loader = torch.utils.data.DataLoader(
    ...
    def load_model(this):
        ...
        if args.net == "WRN":
            ...
            net = "WRN{}-{}-dropout{}".format(
                args.depth, args.width_factor, args.drop_rate
            )
        if args.net == "WRN_madry":
            ...
            net = "WRN_madry{}-{}-dropout{}".format(
                args.depth, args.width_factor, args.drop_rate
            )
        print(net)

        this.model = torch.nn.DataParallel(model)
        this.optimizer = optim.SGD(
            model.parameters(),
            lr=args.lr,
    ...
    def train_model(this):
        ...
        if args.resume:
            ...
            this.start_epoch = checkpoint["epoch"]
            this.model.load_state_dict(checkpoint["state_dict"])
            this.optimizer.load_state_dict(checkpoint["optimizer"])
            this.logger_test = Logger(
                os.path.join(out_dir, "log_results.txt"), title=title, resume=True
            )
        else:
            print("==> Friendly Adversarial Training")
            this.logger_test = Logger(
                os.path.join(args.out_dir, "log_results.txt"), title=title
            )
            this.logger_test.set_names(
    ...
    def get_infos(this):
        ...
        for epoch in range(this.start_epoch, args.epochs):
            this.adjust_learning_rate(this.optimizer, epoch + 1)
            train_time, train_loss, bp_count_avg = this.train(
                this.model,
                this.train_loader,
                this.optimizer,
                this.adjust_tau(epoch + 1, args.dynamictau),
            )

            ## Evalutions the same as DAT.
            loss, test_nat_acc = attack.eval_clean(this.model, this.test_loader)
            ...
            this.logger_test.append(
                [epoch + 1, test_nat_acc, fgsm_acc, test_pgd20_acc, cw_acc]
            )

        return epoch, bp_count_avg, test_nat_acc, test_pgd20_acc
...
if __name__ == "__main__":
    ...
    modelTrain.load_data()

    modelTrain.load_model()

    modelTrain.train_model()

    epoch, bp_count_avg, test_nat_acc, test_pgd20_acc = modelTrain.get_infos()

    modelTrain.save_checkpoint(
        {
            "epoch": epoch + 1,
            "state_dict": modelTrain.model.state_dict(),
            "bp_avg": bp_count_avg,
            "test_nat_acc": test_nat_acc,
            "test_pgd20_acc": test_pgd20_acc,
            "optimizer": modelTrain.optimizer.state_dict(),
        }
    )
